...
public class MyDataBase extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE_ENTRIES_Register = "CREATE TABLE " + TABLE_Register+ " (" +COLUMN_Register_ID +
            ...
            " TEXT, " + COLUMN_Register_Password  + " Text , "+ COLUMN_FK_House + " INTEGER,"
            +" FOREIGN KEY("+COLUMN_FK_House+") REFERENCES "+TABLE_House+"("+COLUMN_House_ID+"));";
    ...
    //insertion de donnée dans la base table house
    public Boolean insertdataRegister( String nom,String prenom,String email, String password)
    {
        ...
        contentvalues.put(COLUMN_FK_House,0);
   ...
   public int getlastid() {
       ...
       SQLiteDatabase db = this.getWritableDatabase();
       Cursor resultset = db.rawQuery("Select * from House", null);
       if (resultset != null) {
    ...
    //fonction pour afficher le registre id
    ...
    //fonction pour afficher le registre id
    public String getNamehouse(int id){

        SQLiteDatabase db = this.getWritableDatabase();
        Cursor resultset = db.rawQuery("SELECT housename FROM House,Register  WHERE Register.IDHouseFK = House.houseid AND Register.IDHouseFK = '"+ id +"'" ,null);
        String housename = resultset.getString(1);
        return housename;
    }
/*
    public  Cursor Afficher()
    {
        ...
        Cursor res =  db.rawQuery("select * from "+TABLE_Register,null);
        return  res;
